# ✅ Tenant Fixture - النسخة النهائية المنقحة
...
#### `test_user`
```python
def test_public_user(test_user):
...
#### `test_manager`
```python
def test_manager(test_manager):
    assert test_manager.role == 'manager'
```

#### `test_worker`
```python
def test_worker(test_worker):
...
- `scope='function'` (افتراضي): tenant جديد لكل test (موصى به)
...
```python
# يتم إنشاء tenant في public schema تلقائياً
...
```python
# يتم تشغيل migrations في tenant schema تلقائياً
